...
void print_crtl(cairo_surface_t *surface){
    ...
    cairo_move_to(cr, 25, 350);
    cairo_show_text(cr, "D : Switch white/dark mode");
    cairo_move_to(cr, 25, 370);
    cairo_show_text(cr, "Q : Quitter l'ecran des controles/fermer le test d'oscillation");

    cairo_move_to(cr, 25, 410);
    ...
    set_bg(cr);
    cairo_rectangle(cr, 25, 487, 20, 15);
    cairo_fill(cr);
    cairo_move_to(cr, 47, 500);
    SET_SOURCE_BLACK(cr);
    cairo_show_text(cr, " : Cellule morte");
    cairo_set_source_rgb(cr, 1, 0, 0);
    cairo_rectangle(cr, 25, 507, 20, 15);
    cairo_fill(cr);
    cairo_move_to(cr, 47, 520);
    SET_SOURCE_BLACK(cr);
